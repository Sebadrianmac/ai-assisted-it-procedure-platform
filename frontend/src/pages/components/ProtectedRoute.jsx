...
const ProtectedRoute = ({ children, onUserLoaded }) => {
  ...
  const refreshAccessToken = async () => {
    ...
    try {
      const response = await publicApi.post("/api/auth/token/refresh/", {
  ...
  const ensureAccessToken = async () => {
    ...
    try {
      ...
      if (typeof expirationTime !== "number" || expirationTime <= currentTime) {
        return refreshAccessToken();
      }
  ...
  const authorize = async () => {
    try {
      const hasAccess = await ensureAccessToken();
