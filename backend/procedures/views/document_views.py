...
def document_list(request):
    if request.method == "GET":
        documents = (
            Document.objects.select_related("uploaded_by").order_by("-created_at")
        )
    ...
    validated_data, error_response = validate_document_content(
        request.data,
        request.FILES,
    )
...
def document_detail(request, document_id):
    ...
    if request.method == "PATCH":
        return update_document(request, document)

    return delete_document(document)


def update_document(request, document):
    validated_data, error_response = validate_document_update(
        request.data,
        request.FILES,
    )
    ...
    should_reindex = bool(searchable_fields & validated_data.keys())
...
def delete_document(document):
    if document.procedure_steps.exists():
        return Response(
            {
                "detail": (
                    "Document cannot be deleted because it is linked to procedure steps."
                ),
            },
